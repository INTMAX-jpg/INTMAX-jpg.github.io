...
create table if not exists public.visit_analytics (
  ...
  event_type text not null default 'page_view' check (event_type in ('page_view', 'gallery_load', 'dwell_time')),
  ...
  dwell_page_type text,
  metadata jsonb not null default '{}'::jsonb
);

alter table public.visit_analytics
  drop constraint if exists visit_analytics_event_type_check;

alter table public.visit_analytics
  add constraint visit_analytics_event_type_check
  check (event_type in ('page_view', 'gallery_load', 'dwell_time'));

alter table public.visit_analytics
  add column if not exists dwell_time_ms integer,
  add column if not exists dwell_page_type text;
...
create index if not exists visit_analytics_dwell_type_created_at_idx
  on public.visit_analytics (dwell_page_type, created_at desc)
...
alter table public.bot_visit_logs
  drop constraint if exists bot_visit_logs_event_type_check;

alter table public.bot_visit_logs
  add constraint bot_visit_logs_event_type_check
  check (event_type in ('page_view', 'gallery_load', 'dwell_time'));
...
select
  ...
  avg(dwell_time_ms) filter (where event_type = 'dwell_time' and dwell_page_type = 'home') as avg_home_dwell_ms,
  avg(dwell_time_ms) filter (where event_type = 'dwell_time' and dwell_page_type = 'post') as avg_post_dwell_ms,
  avg(dwell_time_ms) filter (where event_type = 'dwell_time' and dwell_page_type = 'gallery') as avg_gallery_dwell_ms
...
create table if not exists public.visit_analytics_rollup (
  ...
  avg_home_dwell_ms numeric,
  avg_post_dwell_ms numeric,
  avg_gallery_dwell_ms numeric,
...
alter table public.visit_analytics_rollup
  add column if not exists avg_home_dwell_ms numeric,
  add column if not exists avg_post_dwell_ms numeric,
  add column if not exists avg_gallery_dwell_ms numeric;
...
begin
  ...
  dwell_times as (
    ...
    where event_type = 'dwell_time'
      ...
      and dwell_time_ms between 1000 and 1800000
      and dwell_page_type in ('home', 'post', 'gallery')
  ),
  visitor_regions as (
  ...
  payload as (
    select
      ...
      (select avg(dwell_time_ms) from dwell_times where dwell_page_type = 'home') as avg_home_dwell_ms,
      (select avg(dwell_time_ms) from dwell_times where dwell_page_type = 'post') as avg_post_dwell_ms,
      (select avg(dwell_time_ms) from dwell_times where dwell_page_type = 'gallery') as avg_gallery_dwell_ms,
  ...
  upserted as (
    ...
    on conflict (id) do update set
      ...
      avg_home_dwell_ms = excluded.avg_home_dwell_ms,
      avg_post_dwell_ms = excluded.avg_post_dwell_ms,
      avg_gallery_dwell_ms = excluded.avg_gallery_dwell_ms,
  ...
  select jsonb_build_object(
    ...
    'avg_home_dwell_ms', avg_home_dwell_ms,
    'avg_post_dwell_ms', avg_post_dwell_ms,
    'avg_gallery_dwell_ms', avg_gallery_dwell_ms,
    'regions', regions,
    'countries', countries,
    'cities', cities,
    'devices', devices,
...
select jsonb_build_object(
  ...
  'avg_home_dwell_ms', (select avg_home_dwell_ms from rollup),
  'avg_post_dwell_ms', (select avg_post_dwell_ms from rollup),
  'avg_gallery_dwell_ms', (select avg_gallery_dwell_ms from rollup),
...
live_payload as (
  select jsonb_build_object(
    ...
    'avg_home_dwell_ms', (select avg(dwell_time_ms) from dwell_times where dwell_page_type = 'home'),
    'avg_post_dwell_ms', (select avg(dwell_time_ms) from dwell_times where dwell_page_type = 'post'),
    'avg_gallery_dwell_ms', (select avg(dwell_time_ms) from dwell_times where dwell_page_type = 'gallery'),
    'visitor_location', coalesce((select jsonb_build_object('country', country, 'region', region, 'city', city) from visitor_location), '{}'::jsonb),
